...
namespace ProphetsWay.Logger.Test
{
	public class DestinationTests
	{
		...
		[Theory]
		[InlineData(LogLevels.NoLogging, 1, 1, 2, 2, 2, 2)]
		[InlineData(LogLevels.Error, 0, 1, 1, 1, 1, 1)]
		[InlineData(LogLevels.Warning, 0, 0, 2, 1, 1, 1)]
		[InlineData(LogLevels.Security, 0, 0, 0, 2, 1, 1)]
		[InlineData(LogLevels.Information, 0, 0, 0, 0, 2, 1)]
		[InlineData(LogLevels.Debug, 0, 0, 0, 0, 0, 2)]
		[InlineData(LogLevels.WarningOnly, 0, 0, 2, 0, 0, 0)]
		[InlineData(LogLevels.SecurityOnly, 0, 0, 0, 2, 0, 0)]
		[InlineData(LogLevels.InformationOnly, 0, 0, 0, 0, 2, 0)]
		[InlineData(LogLevels.DebugOnly, 0, 0, 0, 0, 0, 2)]
		...
		[InlineData(LogLevels.WarningOnly)]
		[InlineData(LogLevels.SecurityOnly)]
		[InlineData(LogLevels.InformationOnly)]
		[InlineData(LogLevels.DebugOnly)]
		...
		private void InitLogger()
		{
			...
			var wod = new EventDestination(LogLevels.WarningOnly);
			wod.LoggingEvent += (sender, args) => _wCount++;

			var sod = new EventDestination(LogLevels.SecurityOnly);
			sod.LoggingEvent += (sender, args) => _sCount++;

			var iod = new EventDestination(LogLevels.InformationOnly);
			iod.LoggingEvent += (sender, args) => _iCount++;

			var dod = new EventDestination(LogLevels.DebugOnly);
			dod.LoggingEvent += (sender, args) => _dCount++;
			...
			Utilities.Logger.AddDestination(wod);
			Utilities.Logger.AddDestination(sod);
			Utilities.Logger.AddDestination(iod);
			Utilities.Logger.AddDestination(dod);
		}
